...
export async function getStaticProps({ params }: { params: { bandId: number } }) {
	const { bandId } = params;
	let band = null;
	let error = null;
	try {
...
export default function BandPage({
	...
	if (error) return <QueryError message={`Could not retrieve band data: ${error}`} />;
		...
		<Box m={5} pt={5} textAlign='center'>
			{!band ? (
				...
				<VStack display={band ? 'inherit' : 'none'}>
					...
					<Text fontStyle='italic' color='gray.300' fontFamily='Lato' fontSize='sm'>
						photo by{' '}
